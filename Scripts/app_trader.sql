...
SELECT 
	...
	T.PRICE_PLAY,
	CASE
	WHEN T.PRICE_APP = T.PRICE_PLAY THEN T.APP_PRICE_CALCULATED
	WHEN T.PRICE_APP > T.PRICE_PLAY THEN T.APP_PRICE_CALCULATED
	WHEN T.PRICE_PLAY > T.PRICE_APP THEN T.PLAY_PRICE_CALCULATED 
	END PRICE_OF_APP
FROM 
(SELECT 
	...
	CAST(A.PRICE AS MONEY) PRICE_APP,
	CAST(P.PRICE AS MONEY) PRICE_PLAY,
	CASE WHEN A.PRICE <= 1 THEN 10000 
	ELSE A.PRICE*1000 END APP_PRICE_CALCULATED,
	CASE WHEN CAST(REPLACE(P.PRICE,'$','') AS NUMERIC) <= 1 THEN 10000 
	ELSE CAST(REPLACE(P.PRICE,'$','') AS NUMERIC)*1000 END  PLAY_PRICE_CALCULATED 
FROM 
   APP_STORE_APPS A
   FULL JOIN PLAY_STORE_APPS P ON UPPER(TRIM(A.NAME)) =UPPER(TRIM(P.NAME))) T
 
---WHERE  APP_IN_PLAY ='YES'
 ORDER BY 1
