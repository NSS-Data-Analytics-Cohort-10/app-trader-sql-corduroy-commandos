...
SELECT 
	...
	CASE 
		WHEN UPPER(A.NAME) = UPPER(P.NAME) THEN 'BOTH'
	...
	CASE 
		WHEN CAST(A.REVIEW_COUNT AS INT) IS NULL THEN P.REVIEW_COUNT
		WHEN P.REVIEW_COUNT IS NULL THEN CAST(A.REVIEW_COUNT AS INT)
		ELSE '999999' 
		END
			AS REVIEW_COUNT
...
SELECT
	ALL_APPS, LOCATION, REVIEW_COUNT,
	CASE
...
SELECT
	ALL_APPS, PRICE_COMBINED, LOCATION, RATING_COMBINED, REVIEW_COUNT,
	
	CASE
		WHEN PRICE_COMBINED = CAST('0.00' AS MONEY) THEN CAST('10000.00' AS MONEY)
...
SELECT
	...
	LOCATION, ALL_APPS, PRICE_COMBINED, PURCHASE_PRICE, MARKETING_SPEND, MONTHLY_EARNINGS, 
	
	RATING_COMBINED AS OVERALL_RATING, REVIEW_COUNT, PROJECTED_LIFESPAN,
	
	((MONTHLY_EARNINGS - MARKETING_SPEND) * (PROJECTED_LIFESPAN * 12) - PURCHASE_PRICE) 
		AS NET_PROFIT
